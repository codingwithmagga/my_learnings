...
public:
    ...
    void Produce(const T& t)
    {
        ...
        size_t headIndex = iHead.load(); // Load the value of iHead
        auto endDel = std::next(lst.begin(), headIndex); // Create an iterator to the correct position
        lst.erase(lst.begin(), endDel);
    }
    ...
    bool Consume(T& t)
    {
        ...
        if (iFirst != iTail) {
            ...
            auto it = lst.begin();
            std::advance(it, iHead.load()); // Advance the iterator to the i-th position
            t = *it;
            return true;
        }
